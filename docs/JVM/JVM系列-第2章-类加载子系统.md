...
<clinit的字节码结构> 在字节码文件中的结构类似于下面这种形式:
...
上面这个字节码片段确实代表了`<clinit>`方法，也就是Java类的静态初始化器。在Java编程语言中，`<clinit>`方法是由编译器自动生成的，用来包含类中所有的静态变量赋值和静态代码块中的语句。

### 解析这段字节码
这段字节码是通过编译后的Java类文件中提取的，其组成部分说明如下：

- `static {}`：表示这是一个静态代码块。
- `descriptor: ()V`：这是方法描述符，`()` 表示方法没有参数，`V` 表示方法返回void。
- `flags: ACC_STATIC`：表明这是一个静态方法。对于`<clinit>`，这是标准的，因为它总是静态的。
- `Code:`：后面跟着的是方法的具体指令。
  - `stack=1, locals=0, args_size=0`：这说明执行这段代码的最大堆栈深度是1，局部变量表中的槽数量是0，参数数量是0。这对于`<clinit>`方法来说是典型的，因为它不接受任何参数，也不直接使用局部变量。
  - 指令部分：
    - `0: iconst_1`：将int类型的常数1推送至栈顶。
    - `1: putstatic #5`：将栈顶int型数值（1）存入静态字段。`#5`是对常量池的引用，具体指向一个字段，这里标记为`count:I`，即某个类的名为`count`的int型静态字段。
    - `4: return`：从方法返回。
- `LineNumberTable:`：这部分提供源代码行号与字节码之间的对应关系，帮助调试。
  - `line 17: 0`：表示字节码中的第0条指令对应源代码的第17行。

### `<clinit>`方法的作用
`<clinit>`方法是在类首次被加载到Java虚拟机时执行的，它负责初始化类变量。这个方法是在类加载的初始化阶段被自动触发执行的，确保了类变量在类被首次使用前已经被正确设置。

### 总结
这段字节码展示了一个典型的`<clinit>`方法的结构，它对静态变量进行初始化。每当类被加载且需要进行静态字段初始化时，JVM就会执行这段代码。
...
*   我们尝试获取引导类加载器，获取到的值为 null ，这并不代表引导类加载器不存在，**因为引导类加载器是 C/C++ 语言实现的，我们获取不到**
*   两次获取系统类加载器的值都相同：sun.misc.Launcher$AppClassLoader@18b4aac2 ，这说明**系统类加载器是全局唯一的**

关于“系统类加载器是全局唯一”的表述，这里的理解需要一定的澄清：

- **在单个Java应用程序（JVM实例）中**：系统类加载器通常是唯一的，因为每个Java应用在启动时会创建一个系统类加载器实例来加载所有用户类。这就是为什么在同一应用程序中多次查询系统类加载器会得到相同结果的原因。
- **跨不同的Java应用程序（不同的JVM实例）**：每个应用程序都会有自己的类加载器实例，所以从这个角度看，系统类加载器不是“全局唯一”的，因为不同的JVM实例彼此独立。
...
4.  加载扩展类和应用程序类加载器，并作为他们的父类加载器， 或者说作为其他类加载器（如扩展类加载器和应用程序类加载器）的根加载器。
...
在 Java 类加载器的层级结构中，虽然表达方式可能会导致一些混淆，但通常我们会说“父类加载器”来表示加载类的委托机制，而不一定意味着 Java 的继承关系。这是因为类加载器之间的关系更多是基于委托模型，而不是传统的类继承。

虽然 `AppClassLoader` 并不是 `ExtClassLoader` 的子类（即它们不是在 Java 类继承层级中的父子关系），但在类加载的委托模型中，`AppClassLoader` 确实“视”扩展类加载器为其父加载器。这是因为如果 `AppClassLoader` 不能加载某个类，它会委托给扩展类加载器尝试加载。

所以在描述这种关系时，使用“父类加载器”一词并不意味着直接的类继承，而是指在类加载过程中的查找和委托机制。从这个角度来说，将扩展类加载器描述为系统类加载器的“父加载器”是准确的，但需要理解这是基于加载行为的委托关系，而不是传统的面向对象编程中的类继承。
